...
//============================================================================
extern unsigned int minReadsCountInEdgeToBeNotDeadEnd;	// minimum reads count in an edge to be not dead end edge (default: 10)
extern unsigned int minEdgeLengthToBeNotDeadEnd;		// minimum edge length to be not dead end edge (default: 500)
extern unsigned int minReadsCountInEdgeToBe1MinFlow;	// minimum reads count in an edge to be 1 minimum flow (default: 20)
extern unsigned int minEdgeLengthToBe1MinFlow;			// minimum edge length to be 1 minimum flow (default: 1000)
extern unsigned int minReadsLengthTobeReported;			// minimum reads length to be reported (default: 200)
extern bool reportRemovedEdgesToContigs;				// report removed edges to contigs (default: false)
extern bool reportUnoverlappedReadsToContigs;			// report unoverlapped reads to contigs (default: false)
//============================================================================
...
#define CLOCKSTART clock_t begin = clock(); INT64 mem_start = checkMemoryUsage(); \
		cout << "******************************************************************************************" << endl \
		<< "** Currently in file: " << __FILE__ << endl << "** Function start: "<< __FUNCTION__ << "()" << endl \
		<< "******************************************************************************************" << endl;
#define CLOCKSTOP clock_t end = clock();  INT64 mem_end = checkMemoryUsage(); \
		cout << "******************************************************************************************" << endl \
		<< "** Function stop: " << __FUNCTION__ << "(), Elapsed time: " << double(end - begin) / CLOCKS_PER_SEC<< " seconds, Memory usage: " \
		<< mem_end << " - " <<  mem_start << " = "<< mem_end - mem_start << " MB."<< endl \
		<< "******************************************************************************************" << endl<<endl;
...
public:
    ...
    // Get read filenames
    static vector<string> getReadFilenames();

    // Get edge filenames
    static vector<string> getEdgeFilenames();

    // Get output prefix name
    static string getOutputFilenamePrefix();
